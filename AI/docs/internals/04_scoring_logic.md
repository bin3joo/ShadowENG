...
*   **수식**: DTW (Dynamic Time Warping; 동적 시간 왜곡 알고리즘) 적용. `beta=1.2`
    1.  **Smart Cropping**: 시작 단어부터 끝 단어까지의 실제 발화 구간만 잘라내어 앞뒤의 무음이 평균을 깎아먹는 현상을 차단합니다.
    2.  **F0 전처리 및 기준선 0 고정**: F0에 대해 `librosa.pyin`의 `voiced_flag` 정보를 바탕으로 무성음/침묵 구간을 확실히 `0.0`으로 마스킹하고, 중간의 튀는 값들을 **메디안 필터(kernel=5)**로 부드럽게 깎아냅니다.
    3.  **RMS 기준선 0 고정**: RMS(볼륨) 역시 단순 Z-Score를 넘어서, 하위 15% 이하의 극소 에너지는 무음으로 간주하여 `0.0`으로 강제 고정합니다.
    4.  정규화된 `[f0, rms]` 스택 벡터를 Fast DTW 로 비교합니다 (`radius=10`).
    5.  `distance = DTW_Euclidean(ref, user) / path_len`
    6.  `score = 100.0 * exp(-beta * distance)`
